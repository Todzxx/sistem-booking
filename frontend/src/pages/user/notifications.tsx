...
function getStatusColor(status: string): "default" | "danger" | "success" | "warning" | "accent" {
  switch (status) {
    case "APPROVED":
      return "success";
    case "PENDING":
...
export default function NotificationsPage() {
  ...
  const allNotifications = useMemo(() => {
    ...
    const merged = [...sseMapped, ...bookingMapped];
    const seen = new Set<string>();
    return merged.filter((item) => {
      const key = `${item.type}-${item.id}-${item.status}`;
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-4">
        {[1, 2, 3].map((item) => (
          <div key={item} className="h-28 rounded-xl bg-default-100 animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8 max-w-4xl mx-auto py-8 px-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          className="h-11 px-4 rounded-2xl font-black text-xs border-default-200"
          variant="ghost"
