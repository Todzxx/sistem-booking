...
// Halaman login — form email + password, call /auth/login API
// Redirect ke /admin atau / sesuai role setelah sukses
// ============================================================
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.status === "success" || response.data.data) {
        login(response.data.data.token, response.data.data.refreshToken, response.data.data.user);
      ...
      <div className="w-full max-w-md space-y-5 sm:space-y-7">
        <div className="relative z-10 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-black tracking-tighter text-foreground sm:text-4xl">ROOMSYNC</h1>
            <p className="text-muted font-bold uppercase tracking-[0.2em] text-[10px]">Secure Access Portal</p>
          </div>
        </div>
        ...
        <Card className="w-full rounded-xl border border-default-200 bg-surface/60 p-6 shadow-lg backdrop-blur-xl dark:shadow-none sm:p-8">
          <Card.Header className="flex justify-center pb-5 pt-1 px-0 flex-col items-center gap-2">
            <Card.Title className="text-2xl font-black">Welcome Back</Card.Title>
            <Card.Description className="text-muted font-medium text-sm text-center">Please enter your credentials to access the system</Card.Description>
          </Card.Header>
          ...
          <Card.Content className="py-4 px-0">
            <form className="flex flex-col gap-5 sm:gap-6" onSubmit={handleLogin}>
              ...
              <div className="space-y-4">
                <TextField isRequired name="email" type="email">
                  ...
                  <InputGroup className="rounded-2xl border-default-200">
                    ...
                    <InputGroup.Input placeholder="name@company.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </InputGroup>
                  <FieldError />
                </TextField>
              ...
              <Button className="mt-2 h-14 text-base font-black rounded-2xl shadow-xl shadow-primary/30 transition-all active:scale-95 sm:mt-4" isPending={loading} type="submit" variant="primary">
                Sign Into System <ArrowRight className="ml-2" size={20} />
              </Button>
            </form>

            <div className="text-center mt-7 text-sm font-medium text-muted sm:mt-10">
              New to RoomSync?{" "}
              <Link className="text-primary font-black hover:underline underline-offset-4" to="/register">Register Account</Link>
            </div>
          </Card.Content>
        </Card>

        <p className="text-center text-[10px] text-muted font-bold uppercase tracking-widest">Property of RoomSync Operations</p>
      </div>
    </div>
  );
}
