...
const bookingService = {
    ...
    return await db.transaction(async (tx) => {
      ...
      for (const date of bookingDates) {
        ...
        await tx.insert(bookings).values({
          ...
          recurrenceGroupId,
          depositAmount: facility.depositAmount, // Set deposit from facility
        });
      ...
      const bookingsToNotify = Array.isArray(result) ? result : [result];
  ...
  updateStatus: async (id, status, notes) => {
    ...
    return await db.transaction(async (tx) => {
      ...
      const updated = await tx.query.bookings.findFirst({
        where: eq(bookings.id, id),
        with: { facility: true }
      });
  ...
  cancelBooking: async (id, userId, cancelAll = false) => {
    ...
    await notificationService.emitBookingEvent('BOOKING_CANCELLED', cancelledBooking, userId);
