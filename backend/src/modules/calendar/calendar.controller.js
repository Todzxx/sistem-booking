// ============================================================
// FILE: modules/calendar/calendar.controller.js
// Controller Google Calendar — OAuth, create/delete event, generate link
// Menghubungkan booking sistem dengan Google Calendar user
// ============================================================
...
const idParamSchema = Joi.object({ id: Joi.string().guid({ version: 'uuidv4' }).required() });

const calendarController = {
  // GET /calendar/auth — dapatkan URL OAuth Google Calendar
  ...
  // GET /calendar/oauth/callback — callback setelah user authorize
  handleCallback: async (req, res, next) => {
    try {
      ...
      if (!userId) throw new AppError('User ID not found in state', 400);
  ...
  // POST /calendar/events/:id — buat event Google Calendar untuk booking
  ...
  // DELETE /calendar/events/:id — hapus event Google Calendar
  ...
  // GET /calendar/status — cek apakah user sudah connect Google Calendar
  ...
  // GET /calendar/link/:id — generate link Google Calendar (tanpa OAuth)
  generateLink: async (req, res, next) => {
    try {
      const { error, value } = idParamSchema.validate(req.params);
      if (error) throw new AppError(error.details[0].message, 400);

      const booking = await db.query.bookings.findFirst({ where: eq(bookings.id, value.id), with: { facility: true } });
      if (!booking) throw new AppError('Booking not found', 404);

      const link = googleCalendarService.generateGoogleCalendarLink(
        `Booking: ${booking.facility.name}`, `Purpose: ${booking.purpose}`,
